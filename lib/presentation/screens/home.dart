...
class _HomeState extends State<Home> {
  ...
  Future<void> _getInitColors() async {
    var colors = await AppHive.home.get();
  ...
  Widget build(BuildContext context) {
    ...
    var _colorHeight = (MediaQuery.of(context).size.height -
    ...
    return Scaffold(
      bottomNavigationBar: BottomBar(
        ...
        onPlusClick: () => addColorToList(l10n),
  ...
  void addColorToList(AppLocalizations l10n) {
    if (_colorsList.length == maxColorLength) {
      final snackBar = AppSnackBar(
        content: Text(l10n.noMoreColors),
        action: SnackBarAction(
          label: l10n.ok,
          onPressed: () => {},
        ),
